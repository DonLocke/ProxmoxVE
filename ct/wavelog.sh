#!/usr/bin/env bash
source <(curl -s https://raw.githubusercontent.com/DonLocke/ProxmoxVE/add-wavelog-script-testing/misc/build.func)
...
header_info
if [[ ! -f /opt/wavelog ]]; then msg_error "No ${APP} Installation Found!"; exit; fi
RELEASE=$(curl -s https://api.github.com/repos/wavelog/wavelog/releases/latest | grep "tag_name" | awk '{print substr($2, 3, length($2)-4) }')
if [[ ! -f /opt/${APP}_version.txt ]] || [[ "${RELEASE}" != "$(cat /opt/${APP}_version.txt)" ]]; then
  ...
  mv -r /opt/userdata /opt/wavelog/userdata
